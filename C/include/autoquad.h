...
struct autoquad_message {
  ...
  union {
    ...
    struct {
      uint32_t
          time_boot_ms;  /* Timestamp of the component clock since boot time in
                            ms. */
      uint32_t data0[4]; /* Data bits 1-32 for each ESC. */
      uint32_t data1[4]; /* Data bits 33-64 for each ESC. */
      uint16_t
          status_age
              [4]; /* Age of each ESC telemetry reading in ms compared to boot
                      time. A value of 0xFFFF means timeout/no data. */
      ...
      uint8_t num_in_seq; /* Number of active ESCs in this sequence (1 through
                             this many array members will be populated with
                             data) */
