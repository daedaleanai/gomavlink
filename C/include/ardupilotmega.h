  ...
  union {
    ...
    struct {
      ...
      uint8_t command_id; /* Command Identity (incremental loop: 0 to 255). //A
                             ...
                             pooled just once. */
      uint8_t
          engine_cut_off;  /* Main engine cut-off time before camera trigger (0
                              means no cut-off). */
    ...
    struct {
      ...
      uint8_t target_component; /* Component ID. */
      uint8_t
          session; /* 0: stop, 1: start or keep it up //Session control e.g.
                      show/hide lens. */
      uint8_t
    ...
    struct {
      int32_t input_a; /* Pitch (centi-degrees) or lat (degE7), depending on
                          mount mode. */
      int32_t
          input_b;     /* Roll (centi-degrees) or lon (degE7) depending on mount
                          mode. */
      ...
      uint8_t target_component; /* Component ID. */
      uint8_t
          save_position; /* If "1" it will save current trimmed position on
    ...
    struct {
      ...
      uint8_t
          breach_status; /* Breach status (0 if currently inside fence, 1 if
                            outside). */
    ...
    struct {
      ...
      enum LIMIT_MODULE mods_enabled; /* (uint8_t) AP_Limit_Module bitfield of
                                         enabled modules. */
      enum LIMIT_MODULE
          mods_required; /* (uint8_t) AP_Limit_Module bitfield of required
                            modules. */
      enum LIMIT_MODULE
          mods_triggered; /* (uint8_t) AP_Limit_Module bitfield of triggered
                             modules. */
    } limits_status;
    ...
    struct {
      uint64_t
          time_usec;    /* Image timestamp (since UNIX epoch), as passed in by
    ...
    struct {
      ...
      uint8_t
          completion_mask[10]; /* Bitmask of sphere sections (see
    ...
    struct {
      ...
      float orientation_confidence; /* EXTENSION Confidence in orientation
                                       (higher is better). */
      enum MAV_SENSOR_ORIENTATION old_orientation; /* (uint8_t) EXTENSION
                                                      orientation before
                                                      calibration. */
      enum MAV_SENSOR_ORIENTATION new_orientation; /* (uint8_t) EXTENSION
                                                      orientation after
                                                      calibration. */
    } mag_cal_report;
    ...
    struct {
      ...
      float
          expected_travel_distance; /* Distance the aircraft expects to travel
    ...
    struct {
      uint32_t
          utcTime; /* UTC time in seconds since GPS epoch (Jan 6, 1980). If
      ...
      int32_t baroAltMSL; /* Barometric pressure altitude (MSL) relative to a
                             ...
                             set to INT32_MAX */
      uint32_t
          accuracyHor; /* Horizontal accuracy in mm (m * 1E-3). If unknown set
                          to UINT32_MAX */
      uint16_t accuracyVert; /* Vertical accuracy in cm. If unknown set to
                                UINT16_MAX */
      uint16_t
          accuracyVel; /* Velocity accuracy in mm/s (m * 1E-3). If unknown set
                          to UINT16_MAX */
      int16_t
